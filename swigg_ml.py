...
class SWG(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict, tr_edge_label_index):
        ...
        z = torch.cat([x_dict['restaurant'][row], x_dict['area'][col]], dim=-1)
...
def create_loader(train_data, val_data, test_data):
    train_loader = LinkNeighborLoader(
        ...
        edge_label=train_data['restaurant', 'to', 'area'].edge_label,
        edge_label_index=(('restaurant', 'to', 'area'),
            train_data['restaurant', 'to', 'area'].edge_label_index),
            batch_size=2,
        )
...
def init_model(model, loader):
    ...
    print(f"batch:{batch}")
    batch = batch.to('cpu')
    model(batch.x_dict, batch.edge_index_dict, batch[('restaurant', 'to', 'area')].edge_label_index)
            ...
            for batch_idx, batch in pbar:
                ...
                try:
                    out, _ = model(batch.x_dict,
                            ...
                            batch[('restaurant', 'to', 'area')].edge_label_index
                    )
                except RuntimeError as e:
                    print(f"Error in model forward pass: {str(e)}")
                    continue
                true_label = batch[('restaurant', 'to', 'area')].edge_label
    ...
    def test(loader, epoch):
        ...
        try:
            for batch_idx, batch in pbar:
                ...
                #print(f"pred:{pred}")
                pred.cpu()
                true_label = batch[('restaurant', 'to', 'area')].edge_label.cpu()
                true_label_size = len(true_label)
